...
// ── 메일함 (삭제됨) ─────────────────────────────────────────
// 메일함 화면은 없앴지만, 저장돼 있던 메일 문구·보낸 기록·발송 설정은 지우지 않는다.
// 클라우드 문서는 통째로 덮어쓰므로 읽은 값을 그대로 되돌려 둔다.
const MAIL_KEEP_FIELDS = ['mailTemplates', 'mailLog', 'mailConfig'];
function keepMailFields(remote) {
  const kept = {};
  for (const field of MAIL_KEEP_FIELDS) {
    if (remote && remote[field] !== undefined) kept[field] = remote[field];
  }
  return kept;
}
...
function openEditor(item) {
  editingId = item.id;
  sizeEditorFor(item.category || '기타');
...
$('#editCategory').addEventListener('change', () => sizeEditorFor($('#editCategory').value));
