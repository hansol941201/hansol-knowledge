...
await popup.waitForFunction(() => [...document.querySelectorAll('#messages .bubble')].some(b => b.textContent.includes('클라우드 연동 대기 중')), null, { timeout: 10000 }).catch(() => {});
seen = await bubbles(popup);
check('오프라인: 로컬 저장 후 대기 안내', seen.some(t => t.includes('로컬 저장 완료·클라우드 연동 대기 중')), seen.at(-1)?.split('\n')[0]);
...
const order = await site.evaluate(() => [...document.querySelectorAll('.memory-item')].map(node => {
  const memory = memories.find(m => m.id === node.dataset.memoryId);
  return memory ? savedMillis(memory) : 0;
}));
